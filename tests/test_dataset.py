...
def test_dataset_compare_text_1():
    ...
    result = [
        '  I', '  am', '- testing', '?       -\n', '+ testin',
        '  this', '  algorithm', '  and', '  I', '+ hopit',
        '- hope', '- it', '  will', '  no', '  raise', '  an',
        '- error', '+ aerror', '? +\n'
        ]
...
def test_dataset_differ_list():
    ...
    df = obj.compute_differ_list(col_1='prompt',
                            col_2='asr_transcript',
                            inplace=False
                            )
...
def test_dataset_outliers_mask_predict_mode():
    ...
    assert None is obj.determine_outliers_mask(tol=.2)

def test_dataset_outliers_mask_train_mode():
    obj = Dataset(DF_TEST, mode='train')
    outliers = obj.determine_outliers_mask(tol=.2)
    result = [True] * 49
